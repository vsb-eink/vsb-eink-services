...
export const hostedRoutes = async (app, opts) => {
    ...
    app.route({
        method: 'GET',
        url: '/core/files*',
        onRequest: app.auth([verifyJWT, [verifyRole(Role.ADMIN), verifyScope(Scope.HOSTED_READ)]]),
        handler: async (request, reply) => {
            const path = extractWildcardParam(request);
    ...
    app.route({
        ...
        url: '/core/files*',
        onRequest: app.auth([verifyJWT, [verifyRole(Role.ADMIN), verifyScope(Scope.HOSTED_WRITE)]]),
        handler: async (request, reply) => {
            const path = extractWildcardParam(request);
            return reply.from(joinUrl(HOSTER_URL, '/core/files', path));
        },
